    ...
    try {
        ...
        servicesListDiv.innerHTML = services.map(s => `
            <li class="service-item">
                <img src="${s.imageUrl}" alt="${s.nome}">
                <div class="service-details">
                    <h3>${s.nome}</h3>
                    <p><strong>Preço Base:</strong> R$ ${s.precoBase.toFixed(2)}</p>
                    <p><strong>Duração:</strong> ${s.duracaoEmMinutos} min</p>
                </div>
                <div class="service-actions">
                    <button class="edit-btn" data-id="${s._id}">Editar</button>
                    <button class="delete-btn btn-delete" data-id="${s._id}">Excluir</button>
                </div>
            </li>
        `).join('');
    } catch (error) {
